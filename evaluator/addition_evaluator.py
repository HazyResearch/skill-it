...
class AdditionEvaluator(AbstractEvaluator):
    ...
    def _set_method_str(self, method_name):
        ...
        if self.args.lr != 5e-5:
            method_name += f"_lr_{self.args.lr}"
    ...
    def _evaluate_val(self, tokenized_data, output_idxs, counter, weights):
        ...
        for i, data in tqdm(enumerate(val_dataloader)):
            ...
            predictions = self._query(0.0, 5, output_idxs, encodings)
        ...
        for k in prediction_dict.keys():
            preds_per_skill = prediction_dict[k]
            labels_per_skill = labels_dict[k]
            correct_per_skill = np.equal(preds_per_skill, labels_per_skill).astype(int).sum()
            total_per_skill = len(preds_per_skill)
            self.logger.info(f"Accuracy for skill {k}: {correct_per_skill/total_per_skill}")
    ...
    def _query(self, temperature, max_tokens, output_idxs, encodings):
        ...
        predictions = [gen_text.split("Output: ")[-1][0] for gen_text in gen_texts]
